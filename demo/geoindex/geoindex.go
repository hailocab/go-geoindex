...
func worldCapitals() []index.Point {
	file, err := os.Open("static/capitals.csv")
...
func points(w http.ResponseWriter, r *http.Request) {
	...
	visiblePoints := getIndex().Within(index.NewGeoPoint("topLeft", topLeftLat, topLeftLon), index.NewGeoPoint("bottomRight", bottomRightLat, bottomRightLon))
